...
test("estimates images and tool calls with fixed weights", () => {
  assert.equal(estimateMessageTokens(userMessage("x".repeat(40))), 10);
  assert.equal(estimateMessageTokens({ role: "user", content: [{ type: "image_url", image_url: { url: "data:image/png;base64,AAAA" } }] }), 1024);
