 ...
 * once at activation; toggling `orvixCopilot.inlineSuggestions` is honored on
 * the fly.
 */

import * as vscode from "vscode";
import { ChatCompletionEngine } from "./engine";
import { OrvixInlineCompletionProvider } from "./provider";
...
import { extensionUserAgent, orvixHeaders, ORVIX_ENDPOINTS } from "../transport/protocol";
...
export function registerInlineCompletions(context: vscode.ExtensionContext, deps: InlineCompletionsDeps): vscode.Disposable {
  ...
  const engine: CompletionEngine = {
    ...
    async complete(ctx: CompletionContext, signal: AbortSignal): Promise<CompletionResult> {
      ...
      if (!apiKey) {
        log("[completions] no Orvix API key — skipping");
        return { text: undefined, durationMs: 0 };
      }
      const keyed = new ChatCompletionEngine({
        url: ORVIX_ENDPOINTS.chat,
        headers: orvixHeaders(apiKey, "text/event-stream, application/json", extensionUserAgent(deps.version, deps.vscodeVersion)),
  ...
  const provider = new OrvixInlineCompletionProvider({
    engine,
