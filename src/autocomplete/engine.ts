...
export class ChatCompletionEngine {
  ...
  async complete(context: CompletionContext, signal: AbortSignal): Promise<CompletionResult> {
    ...
    try {
      ...
      throw new Error(`Orvix completion request failed: ${messageOf(error)}`);
    }
    if (!response.ok) {
      // Upstream error bodies can echo prompt context; never surface them.
      await response.body?.cancel().catch(() => undefined);
      throw new Error(`Orvix completion request failed (${response.status})`);
    }
    if (!response.body) throw new Error("Orvix returned an empty completion stream");
