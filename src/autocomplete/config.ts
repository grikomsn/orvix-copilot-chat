 ...
 *
 * Suggestions are strictly opt-in: `orvixCopilot.inlineSuggestions` defaults
 * to false and the provider checks the setting live on every request, so
 * toggling needs no reload.
 */

export const CONFIG_SECTION = "orvixCopilot";
...
/**
 * Live-measured zero-reasoning default (2150ms TTFB, correct FIM output at
 * `reasoning_effort: "none"`). Marginal for ghost text; the only Orvix model
 * measured clean.
 */
export const DEFAULT_INLINE_MODEL = "orvix/deepseek-v4-pro";
